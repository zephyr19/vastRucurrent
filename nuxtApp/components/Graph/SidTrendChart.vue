  ...
  <div :id="cid">
    <div class="control">
      <label style="margin-left: 5px"
        >{{ originData.category == 'static' ? 'SS' : 'MS' }}-{{
          originData.sid
        }}</label
      >
      <label style="margin-left: 10px"
        >Time: {{ originData.timeRange.begintime }} -
        {{ originData.timeRange.endtime }}</label
      >
      <label style="margin-left: 10px"
        >Inteval: {{ interval == 'hour' ? 'By 1 hour' : 'By 1 minute' }}</label
      >
    ...
    <div class="dialog">
      <el-dialog :visible.sync="srScatterVisible" width="70%">
...
export default {
  ...
  methods: {
    ...
    drawChartBySid() {
      ...
      let basedata = [
        { date: x.domain()[0], value: 14.6 },
        { date: x.domain()[1], value: 14.6 },
      ]
      var xAxis = d3
        .axisBottom(x)
    ...
    addAxes(g, xAxis, yAxis, margin, chartWidth, chartHeight) {
      let axes = g.append('g').attr('clip-path', 'url(#axes-clip)')
      axes
        .append('g')
        .attr('class', 'x axis')
    ...
    drawSidPath(g, data, x, y) {
      ...
      g.append('path')
        ...
        .style('fill', (d) => {
          if (this.originData.category == 'static')
            return 'rgba(224, 4, 255, 0.6)'
          else return 'rgba(54,95,139, 0.6)'
        })
        .style('opacity', 0.5)
        .style('stroke', (d) => {
          if (this.originData.category == 'static')
    ...
    remove() {
      this.$root.eventHub.$emit('removeSidTrendChart', {
        category: this.originData.category,
        sid: this.originData.sid,
      })
    },
    showGeoInfo() {
      this.$root.eventHub.$emit('showPosition', {
        category: this.originData.category,
        sid: this.originData.sid,
      })
    },
  ...
  computed: {
    ...
    std: function () {
      if (this.originData) {
        return Math.round(
          d3.mean(this.originData.data, (d) => d.std),
          2
        )
      }
      return null
    ...
    interval: function () {
      let begin =
        this.originData.timeRange.begintime || this.defaultTimeRange.begintime
      let end =
        this.originData.timeRange.endtime || this.defaultTimeRange.endtime
